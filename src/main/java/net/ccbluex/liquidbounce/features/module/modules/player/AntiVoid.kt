...
import net.ccbluex.liquidbounce.utils.PlayerUtils.checkVoid
...
@ModuleInfo(
    ...
    description = "Pull you up when you fall into the void"
)
class AntiVoid : Module() {
    ...
    private val pullbackTime = FloatValue("HypixelPullTime", 1000f, 2000f, 1500f)
    ...
    private val pullTimer: MSTimer = MSTimer()
    private var packets = mutableListOf<Packet<INetHandlerPlayServer>>()

    //Hypixel
    var lastGroundPos: DoubleArray = DoubleArray(3)
    private val timer: TimeHelper = TimeHelper()

    //skid
    private var saveMe = false
    ...
    fun onPacket(event: PacketEvent) {
        ...
        when (modeValue.get().lowercase()) {
            "hypixel" -> {
                if (packets.isNotEmpty() && mc.thePlayer.ticksExisted < 100) packets.clear()
                ...
                if (packet is C03PacketPlayer) {
                    if (checkVoid()) {
                        ...
                        if (packets.isNotEmpty()) {
                            alert("Release Packets - " + packets.size)
                ...
                if (packet is S08PacketPlayerPosLook && packets.size > 1) {
                    alert("Pullbacks Detected, clear packets list!")
                    packets.clear()
                }
            }
    ...
    fun onMove(event: MoveEvent) {
        if (saveMe && pullTimer.hasTimePassed(150L) || mc.thePlayer.isCollidedVertically) {
            saveMe = false
            pullTimer.reset()
        }

        val dist = maxFallDistValue.get()
        if (mc.thePlayer.fallDistance > dist && !LiquidBounce.moduleManager.getModule(Fly::class.java)!!.state && (!voidOnly.get() || !isBlockUnder())) {
            if (!saveMe) {
                saveMe = true
                pullTimer.reset()
            }
    ...
    private fun isBlockUnder(): Boolean {
        return if (mc.thePlayer.posY < 0.0) {
            false
        } else {
            var off = 0
