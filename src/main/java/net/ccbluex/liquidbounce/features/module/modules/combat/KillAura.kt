...
@ModuleInfo(
    ...
    description = "Automatic hit your target"
)
@Native
class KillAura : Module() {
    ...
    private val minCPS: IntegerValue = object : IntegerValue("MinCPS", 14, 1, 20) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val i = maxCPS.get()
            if (i < newValue) set(i)
    ...
    private val swingRangeValue = object : FloatValue("SwingRange", 4.1f, 0f, 15f) {
    ...
    val rangeValue = object : FloatValue("AttackRange", 4.1f, 1f, 8f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
            val i = discoverRangeValue.get()
            if (i < newValue) set(i)
        }
    }
    private val throughWallsRangeValue = object : FloatValue("ThroughRange", 2.0f, 0f, 8f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
    ...
    private val priorityValue = ListValue(
        "Priority",
        arrayOf("Distance", "Health", "Fov", "Armor", "HurtResistantTime"),
        "Distance"
    )
    ...
    private val autoBlockRangeValue = object : FloatValue("AutoBlockRange", 4.1f, 0f, 8f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
            val i = discoverRangeValue.get()
            if (i < newValue) set(i)
        }
    ...
    private val autoBlockPacketValue = ListValue(
        "AutoBlockPacket",
        arrayOf("AfterTick", "AfterAttack", "Vanilla","Hypixel","NCP","Vulcan"),
        "AfterTick"
    ).displayable { autoBlockValue.equals("Range") }
    ...
    private val NoScaffold = BoolValue("NoScaffold",true)

    // Rotations
    private val rotationModeValue = ListValue(
        "RotationMode",
        arrayOf("None", "LiquidBounce", "ForceCenter", "SmoothCenter", "SmoothLiquid", "LockView","BackTrack"),
        "LiquidBounce"
    )

    private val maxTurnSpeed: FloatValue = object : FloatValue("MaxTurnSpeed", 90.8f, 1f, 180f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
    ...
    private val minTurnSpeed: FloatValue = object : FloatValue("MinTurnSpeed", 90.8f, 1f, 180f) {
        override fun onChanged(oldValue: Float, newValue: Float) {
    ...
    private val rotationSmoothValue =
        FloatValue("CustomSmooth", 1f, 1f, 10f).displayable { rotationSmoothModeValue.equals("Custom") }
    ...
    private val hitableValue = BoolValue("Hitable", true).displayable { !rotationModeValue.equals("None") }
    ...
    // Bypass
    private val failRateValue = FloatValue("FailRate", 10f, 0f, 100f)
    ...
    private val switchDelayValue =
        IntegerValue("SwitchDelay", 1, 1, 2000).displayable { targetModeValue.equals("Switch") }
    private val limitedMultiTargetsValue =
        IntegerValue("LimitedMultiTargets", 0, 0, 50).displayable { targetModeValue.equals("Multi") }

    // Visuals
    private val markValue = ListValue("Mark", arrayOf("Liquid", "Circle", "Block", "Jello", "Sims", "None"), "Circle")
                ...
                "circle" -> {
                    ...
                    for (i in 0..360 step 10) {
                        RenderUtils.glColor(
                            Color.getHSBColor(
                                if (i < 180) {
                                    Rainbow.rainbowStart.get() + (Rainbow.rainbowStop.get() - Rainbow.rainbowStart.get()) * (i / 180f)
                                } else {
                                    Rainbow.rainbowStart.get() + (Rainbow.rainbowStop.get() - Rainbow.rainbowStart.get()) * (-(i - 360) / 180f)
                                }, 0.7f, 1.0f
                            )
                        )
    ...
    private fun attackEntity(entity: EntityLivingBase) {
        ...
        if (mc.thePlayer.isBlocking || (autoBlockValue.equals("Range") && canBlock)) {
            ...
            if(autoBlockPacketValue.equals("Vulcan")) {
            ...
            if (autoBlockPacketValue.get().equals("Hypixel", true)) {
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        if (rotationModeValue.get().equals("BackTrack", ignoreCase = true)) {
            if (predictValue.get())
