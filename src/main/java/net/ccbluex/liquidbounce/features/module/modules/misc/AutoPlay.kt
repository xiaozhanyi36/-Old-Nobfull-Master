...
@ModuleInfo(name = "AutoPlay", category = ModuleCategory.MISC, description = "Automatically joins a new game when the one you're playing ends")
class AutoPlay : Module() {
    ...
    private fun queueAutoPlay(delay: Long = delayValue.get().toLong() * 1000, runnable: () -> Unit) {
        ...
        if (LiquidBounce.combatManager.resetKillsMode == 1)  {
            LiquidBounce.combatManager.killedEntities = 0
            LiquidBounce.hud.addNotification(Notification("CombatManager", "Kills reset for Game End.", NotifyType.INFO, 3000))
        }
    }
