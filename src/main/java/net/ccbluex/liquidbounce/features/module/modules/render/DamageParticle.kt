...
@ModuleInfo(name = "DamageParticle", category = ModuleCategory.RENDER, description = "Allows you to see targets damage")
        ...
        synchronized(particles) {
            for (entity in mc.theWorld.loadedEntityList) {
                if (entity is EntityLivingBase && EntityUtils.isSelected(entity, true)) {
                    ...
                    val prefix = if (!colorRainbow.get()) (if (lastHealth > entity.health) {
                        "§c❤"
                    } else {
                        "§a§l❤"
        ...
        synchronized(particles) {
            ...
            for (particle in particles) {
                ...
                mc.fontRendererObj.drawStringWithShadow(
                    ...
                    (if (colorRainbow.get()) ColorUtils.rainbowWithAlpha(colorAlphaValue.get()) else Color(
                        colorRedValue.get(),
                        colorGreenValue.get(),
                        colorBlueValue.get(),
                        colorAlphaValue.get()
