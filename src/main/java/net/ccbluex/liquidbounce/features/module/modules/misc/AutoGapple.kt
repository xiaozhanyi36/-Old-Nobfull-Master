...
@ModuleInfo(name = "AutoGapple", category = ModuleCategory.MISC, description = "Auto eat Golden Apple when low HP")
class AutoGapple : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        if (mc.thePlayer.health > healthValue.get() || !timer.hasTimePassed(
                delayValue.get().toLong()
            ) || noAbsorption.get() && mc.thePlayer.absorptionAmount > 0
        ) {
            return
        }
