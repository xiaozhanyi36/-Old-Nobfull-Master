...
class Scaffold : Module() {
    ...
    // Delay
    private val HypixelYawValue: IntegerValue = IntegerValue("HypixelYaw", 180, -360, 360)
    private val HypixelPitchValue = IntegerValue("HypixelPitch", 79, 60, 100)
    private val placeableDelay = ListValue("PlaceableDelay", arrayOf("Normal", "Smart", "OFF"), "Normal")
    private val maxDelayValue: IntegerValue = object : IntegerValue("MaxDelay", 30, 0, 1000) {
    ...
    //counter
    private val counterValue = ListValue("Counter", arrayOf("None", "Simple", "Novoline","IDK","Astolfo"), "Novoline")

    //timer
    private val timerModeValue = ListValue("Timer", arrayOf("None", "Normal", "Dynamic"), "Normal")
    private val timerSpeedValue =
        FloatValue("TimerSpeed", 1.1F, 0.1F, 3.0F).displayable { timerModeValue.get() == "Normal" }
    private val timerMaxValue =
        FloatValue("MaxTimerSpeed", 1.2F, 0.1F, 3.0F).displayable { timerModeValue.get() == "Dynamic" }
    private val timerMinValue =
        FloatValue("MinTimerSpeed", 1.2F, 0.1F, 3.0F).displayable { timerModeValue.get() == "Dynamic" }
    private val timerDelayValue =
        IntegerValue("DelayTimer", 1000, 500, 5000).displayable { timerModeValue.get() == "Dynamic" }
    ...
    private val sprintValue =
        ListValue("Sprint", arrayOf("Always", "Dynamic", "OnGround", "OffGround", "OFF"), "OnGround")
    ...
    // Eagle
    private val eagleValue = ListValue("Eagle", arrayOf("Silent", "Normal", "OFF"), "Silent")
    private val blocksToEagleValue = IntegerValue("BlocksToEagle", 1, 0, 10).displayable { !eagleValue.equals("OFF") }
    ...
    private val rotationsValue =
        ListValue("Rotations", arrayOf("None", "Vanilla", "AAC", "Verus", "Matrix", "Custom","hypixel"), "AAC")
    private val aacYawValue = IntegerValue("AACYawOffset", 35, 0, 90).displayable { rotationsValue.equals("AAC") }
    ...
    private val minRotationSpeedValue: IntegerValue = object : IntegerValue("MinRotationSpeed", 69, 0, 180) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val v = maxRotationSpeedValue.get()
            if (v < newValue) set(v)
        }
    }.displayable { !rotationsValue.equals("None") } as IntegerValue
    private val maxRotationSpeedValue: IntegerValue = object : IntegerValue("MaxRotationSpeed", 69, 0, 180) {
        override fun onChanged(oldValue: Int, newValue: Int) {
    ...
    private val keepLengthValue =
        IntegerValue("KeepRotationTick", 10, 0, 20).displayable { !rotationsValue.equals("None") }
    ...
    private val speedModifierValue = FloatValue("SpeedModifier", 0.8f, 0f, 2f)
    ...
    private val towerTimerValue = FloatValue("TowerTimer", 1.14f, 0.1f, 5f)
    ...
    private val hitableCheck = ListValue("HitableCheck", arrayOf("Simple", "Strict", "OFF"), "OFF")
    ...
    private val jumpMotionValue =
        FloatValue("TowerJumpMotion", 0.39f, 0.3681289f, 0.79f).displayable { towerModeValue.equals("Jump") }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!towerStatus) {
            if (timerModeValue.get() == "Dynamic") {
                if (timerDel.hasTimePassed(timerDelayValue.get().toLong())) {
                    mc.timer.timerSpeed = TimeUtils.randomFloat(timerMinValue.get(), timerMaxValue.get())
                    timerDel.reset()
                }
            }
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        val infoWidth2 = Fonts.minecraftFont.getStringWidth(info + "")
        when (counterValue.get().lowercase()) {
            "idk" -> {
                ...
                Fonts.font40.drawString(
                    info, scaledResolution.scaledWidth / 2,
                    scaledResolution.scaledHeight / 2 + 7, Color.WHITE.rgb
                )
                Fonts.font40.drawString(
                    info, (scaledResolution.scaledWidth / 2.00000002).toInt(),
                    (scaledResolution.scaledHeight / 2.000999999999 + 7).toInt(), Color.BLACK.rgb
                )
                Fonts.font40.drawString(
                    info, (scaledResolution.scaledWidth / 1.99999998).toInt(),
                    (scaledResolution.scaledHeight / 1.9899999 + 7).toInt(), Color.BLACK.rgb
                )
                Fonts.font40.drawString(
                    info, scaledResolution.scaledWidth / 2,
                    scaledResolution.scaledHeight / 2 + 7, Color.PINK.rgb
                )
                if (mc.thePlayer.onGround) {
                    Fonts.font40.drawString(
                        info1, (scaledResolution.scaledWidth / 2.00000002).toInt(),
                        (scaledResolution.scaledHeight / 1.9002001 + 7).toInt(), Color.BLACK.rgb
                    )
                    Fonts.font40.drawString(
                        info1, (scaledResolution.scaledWidth / 1.99999998).toInt(),
                        (scaledResolution.scaledHeight / 1.89998999 + 7).toInt(), Color.BLACK.rgb
                    )
                    Fonts.font40.drawString(
                        info1, scaledResolution.scaledWidth / 2,
                        (scaledResolution.scaledHeight / 1.9 + 7).toInt(), Color.WHITE.rgb
                    )
                } else {
                    Fonts.font40.drawString(
                        info2, (scaledResolution.scaledWidth / 2.00000002).toInt(),
                        (scaledResolution.scaledHeight / 1.9002001 + 7).toInt(), Color.BLACK.rgb
                    )
                    Fonts.font40.drawString(
                        info2, (scaledResolution.scaledWidth / 1.99999998).toInt(),
                        (scaledResolution.scaledHeight / 1.89998999 + 7).toInt(), Color.BLACK.rgb
                    )
                    Fonts.font40.drawString(
                        info2, scaledResolution.scaledWidth / 2,
                        (scaledResolution.scaledHeight / 1.9 + 7).toInt(), Color.GREEN.rgb
                    )
                }

            }
            "astolfo" -> {
                Fonts.font40.drawString("$blocksAmount blocks", scaledResolution.scaledWidth / 2 + 25.toFloat(), scaledResolution.scaledHeight / 2.toFloat(), -1, true)
            }
            ...
            "novoline" -> {
                if (slot in 0..8 && mc.thePlayer.inventory.mainInventory[slot] != null && mc.thePlayer.inventory.mainInventory[slot].item != null && mc.thePlayer.inventory.mainInventory[slot].item is ItemBlock) {
                    ...
                    GlStateManager.translate(
                        scaledResolution.scaledWidth_double / 2 - 22,
                        scaledResolution.scaledHeight_double / 2 + 16,
                        scaledResolution.scaledWidth_double / 2 - 22
                    )
                    RenderHelper.enableGUIStandardItemLighting()
                    mc.renderItem.renderItemIntoGUI(
                        stack,
                        width / 2 - Fonts.font40.getStringWidth("$info Blocks"),
                        (height * 0.6 - Fonts.font40.FONT_HEIGHT * 0.5).toInt()
                    )
                    RenderHelper.disableStandardItemLighting()
                    GlStateManager.popMatrix()
                ...
                Fonts.minecraftFont.drawString(
                    ...
                    scaledResolution.scaledHeight / 2F + 20,
                    -1,
                    true
                )
            }
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        for (i in 0 until (expandLengthValue.get() + 1)) {
            ...
            if (BlockUtils.isReplaceable(blockPos) && placeInfo != null) {
                RenderUtils.drawBlockBox(blockPos, Color(255, 255, 255, 95), false, true, 1f)
                break
            }
        }
    }
        ...
        if (!rotationsValue.equals("None")) {
            lockRotation = when (rotationsValue.get().lowercase()) {
                ...
                "matrix" -> {
                    if (!towerStatus) {
                        Rotation(
                            mc.thePlayer.rotationYaw + if (mc.thePlayer.movementInput.moveForward < 0) 0 else 170,
                            75F
                        )
                    } else {
                        placeRotation.rotation
                ...
                "hypixel" ->{
                    Rotation(mc.thePlayer.rotationYaw + (if (mc.thePlayer.movementInput.moveForward > 0) 180 else 0) + HypixelYawValue.get(),HypixelPitchValue.get().toFloat())
                }
    ...
    private fun isNotSafe(): Boolean {
        if (mc.theWorld.getCollidingBoundingBoxes(
                mc.thePlayer,
                mc.thePlayer.entityBoundingBox.offset(0.0, -0.015625, 0.0).expand(0.0, 0.0, 0.0)
            ).isNotEmpty() || mc.thePlayer.onGround
        ) {
        ...
        if (mc.theWorld.getCollidingBoundingBoxes(
                mc.thePlayer,
                mc.thePlayer.entityBoundingBox.offset(0.0, -2.0, 0.0).expand(0.0, 0.0, 0.0)
            ).isNotEmpty()
        ) {
            return false
        }
    ...
    private fun canCatch(blockPos1: BlockPos, blockPos2: BlockPos): Float {
        return calHeight(
            (abs(blockPos1.x - blockPos2.x) + abs(blockPos1.z - blockPos2.z)).toDouble()
                .roundToInt()
        )
        // 忘记把改过的js传上去了
    }
    ...
    private val isOnEdge: Boolean
        get() = mc.theWorld.getCollidingBoundingBoxes(
            mc.thePlayer,
            mc.thePlayer.entityBoundingBox.offset(mc.thePlayer.motionX * 0.25, -1.0, mc.thePlayer.motionZ * 0.25)
                .expand(-0.15, 0.0, -0.15)
        ).isEmpty() &&
                mc.theWorld.getCollidingBoundingBoxes(
                    mc.thePlayer,
                    mc.thePlayer.entityBoundingBox.offset(0.0, -0.625, 0.0).expand(0.0, 0.0, 0.0)
                ).isNotEmpty()
...
//    }
    ...
    override val tag: String
        get() = if (towerStatus) {
            ...
            "${rotationsValue.get()}-${maxDelayValue.get()}-${minDelayValue.get()}"
        }
