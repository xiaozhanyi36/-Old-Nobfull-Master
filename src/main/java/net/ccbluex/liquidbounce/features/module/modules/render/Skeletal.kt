...
class Skeletal : Module() {
    ...
    fun onRender(event: Render3DEvent) {
        val map = playerModelMap
            .filter {
                (!onlyEnemyValue.get() || EntityUtils.isSelected(
                    it.key,
                    true
                )) && (mc.gameSettings.thirdPersonView != 0 || mc.thePlayer.entityId != it.key.entityId)
            }
    ...
    class ClonedModelPlayer(model: ModelPlayer) {
        ...
        val rightArm = Vector3f(
            model.bipedRightArm.rotateAngleX,
            model.bipedRightArm.rotateAngleY,
            model.bipedRightArm.rotateAngleZ
        )
        val leftArm =
            Vector3f(model.bipedLeftArm.rotateAngleX, model.bipedLeftArm.rotateAngleY, model.bipedLeftArm.rotateAngleZ)
        val rightLeg = Vector3f(
            model.bipedRightLeg.rotateAngleX,
            model.bipedRightLeg.rotateAngleY,
            model.bipedRightLeg.rotateAngleZ
        )
        val leftLeg =
            Vector3f(model.bipedLeftLeg.rotateAngleX, model.bipedLeftLeg.rotateAngleY, model.bipedLeftLeg.rotateAngleZ)
    }
}