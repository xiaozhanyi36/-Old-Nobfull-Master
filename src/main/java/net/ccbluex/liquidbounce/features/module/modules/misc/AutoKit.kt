...
@ModuleInfo(
    name = "AutoKit",
    description = "Automatically selects kits for you in BlocksMC Skywars.",
...
class AutoKit : Module() {
        ...
        override fun onChanged(oldValue: Boolean, newValue: Boolean) {
            if (newValue)
                LiquidBounce.hud.addNotification(
                    Notification(
                        name, "Change default kit by right clicking the kit selector and select.",
                        NotifyType.INFO
                    )
                )
        }
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (clickStage == 2) {
            ...
            if (timeoutTimer.hasTimePassed(kitTimeOutValue.get())) {
                ...
                LiquidBounce.hud.addNotification(
                    Notification(
                        name, "Kit checker timed out. Please use the right kit name.",
                        NotifyType.ERROR
                    )
                )
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is C0DPacketCloseWindow && editMode.get()) {
            editMode.set(false)
            LiquidBounce.hud.addNotification(Notification(name, "Edit mode aborted.", NotifyType.INFO))
        ...
        if (packet is S2FPacketSetSlot) {
            ...
            if (clickStage == 0 && windowId == 0 && slot >= 36 && slot <= 44 && itemName.contains(
                    "bow",
                    true
                ) && displayName.contains("kit selector", true)
            ) { // dynamic for solo/teams
                if (editMode.get()) {
                    listening = true
            ...
            if (text.contains("kit has been selected", true)) {
                if (editMode.get()) {
                    ...
                    LiquidBounce.hud.addNotification(
                        Notification(
                            name,
                            "Successfully detected and added $kitName kit.",
                            NotifyType.SUCCESS
                        )
                    )
                    ...
                    LiquidBounce.hud.addNotification(
                        Notification(
                            name,
                            "Successfully selected ${kitNameValue.get()} kit.",
                            NotifyType.SUCCESS
                        )
                    )
                    debug("finished")
