...
import net.ccbluex.liquidbounce.ui.client.clickgui.clickguis.novoline.AddSettings
...
class ModuleManager : Listenable {
    ...
    val modules1 = TreeSet<Module> { module1, module2 -> module1.name.compareTo(module2.name) }
    ...
    fun getModulesByName(name: String): List<Module> {
        return this.modules.filter { it.name.lowercase().contains(name.lowercase(Locale.getDefault())) }
    }

    fun getEnabledModule(): TreeSet<Module> {
        val modules2: TreeSet<Module> = modules1
        for (i in modules) {
            if (i.state)
                modules2.add(i)
        }
    ...
    fun getEnableModules(): Int {
        var enabled = 0

        for (module in modules) {
            if (module.state) enabled += 1
        }
    ...
    private fun onKey(event: KeyEvent) {
        if (pendingBindModule == null) {
            modules.filter { it.triggerType == EnumTriggerType.TOGGLE && it.keyBind == event.key }
                .forEach { it.toggle() }
        } else {
