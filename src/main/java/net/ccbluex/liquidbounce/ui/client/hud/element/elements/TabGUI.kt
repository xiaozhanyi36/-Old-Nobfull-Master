...
class TabGUI(x: Double = 5.0, y: Double = 25.0) : Element(x = x, y = y) {
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        val color2 = Color(redValue.get(), greenValue.get(), blueValue.get(), alphaValue.get())
        val addColor = color2.rgb + Color(0, 0, 0, 50).rgb
        ...
        RainbowShader.begin(rectangleRainbowEnabled, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
            RenderUtils.drawGradientSideways(1.0, (1 + tabY - 1).toDouble(), width.get().toDouble(), (tabY + tabHeight.get()).toDouble(), addColor, color2.rgb)
        }
        ...
        tabs.forEachIndexed { index, tab ->
            ...
            val textColor = if (selectedCategory == index) 0xffffff else Color(100, 100, 100).rgb
    ...
    private inner class Tab(val tabName: String) {
        ...
        fun drawTab(x: Float, y: Float, color: Int, backgroundColor: Int, borderColor: Int, borderStrength: Float,
                    upperCase: Boolean, fontRenderer: FontRenderer, borderRainbow: Boolean, rectRainbow: Boolean, color2: Int) {
            var maxWidth = 0
            ...
            RainbowShader.begin(rectRainbow, if (rainbowX.get() == 0.0F) 0.0F else 1.0F / rainbowX.get(), if (rainbowY.get() == 0.0F) 0.0F else 1.0F / rainbowY.get(), System.currentTimeMillis() % 10000 / 10000F).use {
                RenderUtils.drawGradientSideways((x - 1.toFloat()).toDouble(), (y + itemY - 1).toDouble(), (x + menuWidth - 2F).toDouble(), (y + itemY + tabHeight.get() - 1).toDouble(), color, color2)
            }

            GlStateManager.resetColor()

            modules.forEachIndexed { index, module ->
                val moduleColor = if (module.state) Color(0, 0, 0).rgb else Color(100, 100, 100).rgb
