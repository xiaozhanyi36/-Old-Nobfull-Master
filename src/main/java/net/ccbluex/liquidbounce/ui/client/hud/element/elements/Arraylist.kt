...
class Arraylist(
    ...
    private val tagsValue = ListValue(
        "TagsStyle",
        arrayOf("-", "|", "()", "[]", "{}", "<>", "=", "<", ">", ":", "~", "Space", "None","->","<-"),
        "Space"
    )
    ...
    private val backgroundModeValue = ListValue("Background-Mode", arrayOf("Normal", "Sideways"), "Normal")
    ...
    private val textYValue = FloatValue("TextY", 2.3255816F, 0F, 20F)
    ...
    private fun getModuleName(module: Module) = if (split.get()) {
        module.splicedName
    } else {
        module.name
    }
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        when (side.horizontal) {
            Horizontal.RIGHT, Horizontal.MIDDLE -> {
                modules.forEachIndexed { index, module ->
                    ...
                    when (backgroundModeValue.get().lowercase()) {
                        "normal" -> {
                        ...
                        "sideways" -> {
                            RenderUtils.drawGradientSideways(
                                ...
                                when (backgroundColorMode.lowercase()) {
                                    "rainbow" -> ColorUtils.hslRainbow(index + 1, indexOffset = 200 * rainbowSpeed.get()).rgb
                                    "skyrainbow" -> ColorUtils.skyRainbow(
                                        index * 2,
                                        saturationValue.get(),
                                        brightnessValue.get(),
                    ...
                    fontRenderer.drawString(
                        ...
                        yPos + textY,
                        ColorUtils.reverseColor(
                            when (tagColorModeValue.get().lowercase()) {
                                "rainbow" -> ColorUtils.reverseColor(
                                ...
                                "anotherrainbow" -> ColorUtils.reverseColor(
                                    ColorUtils.fade(
                                        tagCustomColor,
                                        100,
                                        index + 1
                    ...
                    if (!rectMode.equals("none", true)) {
                        ...
                        when (rectMode.lowercase()) {
                            ...
                            "outline" -> {
                                if (module == modules[0]) RenderUtils.drawRect(xPos - 2, yPos, -1F, yPos - 1, rectColor)
        ...
        if (mc.currentScreen is GuiHudDesigner) {
            ...
            for (module in modules) {
                when (side.horizontal) {
                    Horizontal.RIGHT, Horizontal.MIDDLE -> {
                        val xPos = -module.slide.toInt() - 1
                        if (x2 == Int.MIN_VALUE || xPos < x2) x2 = xPos
                    }
                    Horizontal.LEFT -> {
                        val xPos = module.slide.toInt() + 10
                        if (x2 == Int.MIN_VALUE || xPos > x2) x2 = xPos
                    }
                }
            }
