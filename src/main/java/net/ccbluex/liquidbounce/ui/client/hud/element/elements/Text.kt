...
class Text(
    ...
    val rectValue = ListValue("Rect", arrayOf("Normal", "RNormal", "OneTap", "Skeet", "Destiny", "None"), "None")
    private val rectExpandValue = FloatValue("RectExpand", 0.1F, 0F, 1F)
    private val lrectAlphaValue = IntegerValue("DestinyAlpha", 80, 0, 255)
        ...
        when (rectValue.get().lowercase()) {
            ...
            "destiny" -> {
                RenderUtils.drawRect(
                    ...
                    fontRenderer.getStringWidth(displayText) + expand,
                    fontRenderer.FONT_HEIGHT + expand,
                    Color(0, 0, 0, lrectAlphaValue.get())
                )
                RenderUtils.drawRect(
                    -expand - 1,
                    -expand - 2,
