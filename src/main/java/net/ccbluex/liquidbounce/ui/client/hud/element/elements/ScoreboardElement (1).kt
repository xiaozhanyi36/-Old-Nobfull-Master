...
import net.ccbluex.liquidbounce.features.module.modules.color.HUD.displayString
/**
...
class ScoreboardElement(
    ...
    override fun drawElement(partialTicks: Float): Border? {
                ...
                for (domain in allowedDomains) {
                    if (name.contains(domain, true)) {
                        name = when (serverValue.get().lowercase()) {
                            "clientname" -> LiquidBounce.CLIENT_NAME + "@Silent"
                            "website" -> LiquidBounce.CLIENT_WEBSITE
                            "qqgroup" -> LiquidBounce.QQ_GROUP.toString()
                            "clientversion" -> LiquidBounce.CLIENT_VERSION
                            "custom" -> displayString.get()
        ...
        if (LiquidBounce.theme == "Custom") {
            RenderUtils.drawRect(-maxWidth.toFloat() - 5 - if (rectValue.get()) 3 else 0, -2f, 5f, -1f, HUD.getClientCustomColor())
        } else RenderUtils.drawGradientSideways(-maxWidth.toFloat() - 5 - if (rectValue.get()) 3 else 0, -2f, 5f, -1f, ColorUtils.hslRainbow(10, indexOffset = 30).rgb, ColorUtils.hslRainbow(10, indexOffset = 60).rgb)
