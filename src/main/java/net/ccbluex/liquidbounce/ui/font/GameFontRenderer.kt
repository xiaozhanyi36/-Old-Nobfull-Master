...
class GameFontRenderer(font: Font) : FontRenderer(Minecraft.getMinecraft().gameSettings,
    ...
    fun drawRainbowString(s: String, x: Float, y: Float, indexOffset: Int, shadow: Boolean = false) {
        val array = arrayOf(s.split(""))
        var index = 0
        var index1 = 0
        for (str in array) {
            for (s2 in str) {
                if (s2 != "[" && s2 != "]" && s2 != ",") {
                    drawString(s2, x + index, y, ColorUtils.hslRainbow(index1, indexOffset = indexOffset).rgb, shadow)
                    index += getStringWidth(s2)
                    index1 += 1
                }
            }
        }
